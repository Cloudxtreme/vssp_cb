...
exports.startMotionDetect = function(node_name, node_id, user_id, duration, videoProfileID, videoBatchID, parentBatchID, record_service_type, motion_detect_sensitive_level, detect_motion_on_screen) {
	...
	logger.info('Start Scheduled Motion Detect....implementation...');
	...
	setTimeout(function() {
		...
		checkForMotionDetect(pid, node_id, node_name, user_id, duration, videoProfileID, videoBatchID, parentBatchID, record_service_type, outputFile, executor);
	}, 2000);
...
checkForMotionDetect = function(pid, node_id, node_name, user_id, duration, videoProfileID, videoBatchID, parentBatchID, record_service_type, outputFile, executor) {
	var response = JSON.parse(constants.JSON_RESPONSE);	
	...
	var p = exec(cmd, function(err, stdout, stderr) {
		...
		sendVideoRecordScheduledRequestOptionToServer(node_name, node_id, user_id, duration, videoProfileID, videoBatchID, parentBatchID, record_service_type, 'MOTION DETECTION INPROGRESS', 'Successfully started the record service via Motion Detect');
...
function sendVideoRecordScheduledRequestOptionToServer(node_name, node_id, user_id, duration, videoProfileID, videoBatchID, parentBatchID, record_service_type, status, comments) {
	...
	var service_desc = 'Video record service- Motion Detect starts as per schedule';
	...
	request.post({
		...
		body: dataToPost
			...
			error_info = 'Failed to post the scheduled video service request details with motion-detect enabled to server';
			logger.error(error_info);
		});
}
