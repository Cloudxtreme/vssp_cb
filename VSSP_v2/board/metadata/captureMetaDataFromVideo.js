...
exports.captureImageFromVideo = function(file, node, device_name) {
...
scanAndCreateMetaDataForVideos = function(node) {
	...
	files.forEach(function(file) { 
		...
		if(! isHidden) {
			...
			try {
				...
				if(extn == '.mp4') {
					...
					if(! fs.existsSync(jsonFile)) {					
						//logger.info('meta-data for Video:' + path.basename(abs_file, '.mp4') + ' is NOT found. Need to create now');
						captureImageFromVideo(abs_file, node, device.name);
					} else {
						var videoObject = getJsonObjectFromFile(jsonFile);
						if(videoObject != null) {
							if(videoObject['Duration'] != null) {
								//logger.info('meta-data for Video:' + path.basename(abs_file, '.mp4') + ' is found');
							} else {
								//logger.info('Duration is not found. ReCapturing it again');
								captureImageFromVideo(abs_file, node, device.name);
							}
						} else {
							//logger.info('Video Object is not found. ReCapturing it again');
							captureImageFromVideo(abs_file, node, device.name);
						}
					}
				}
			...
			//logger.info('Looks like the file:' + file + ' is hidden.. So, ignoring it..');
		}
	});
}
