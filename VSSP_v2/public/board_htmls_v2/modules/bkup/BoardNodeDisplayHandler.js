...
authenticateAdminUser = function() {
	setStatus('Admin authentication...');
	dojo.attr(dojo.byId('preloader'), 'style', 'display:block');	
	var auth = new modules.UserAuthentication;
	auth.isUserAdmin(dojo.byId('adminUsername').value, dojo.byId('adminPassword').value, dataStore['auth_id']); 	
}

dojo.subscribe('UserAdminAuthenticationSignal', function(e) {
	...
	try {
		if(e.status === 'SUCCESS') {
			dataStore['admin_id'] = e.result.user_id;
			var adminDlg = dijit.byId('adminAuthenticationDlg');
			adminDlg.hide();
			showInformation('Successfully authenticated as Admin. Pls. continue the task...');
			console.log('Data store is:' + dojo.toJson(dataStore, true));
		} else {
			setStatus(e.error_info);
			dojo.byId('adminLoginError').innerHTML = e.error_info;
		}
	} catch(err) {
