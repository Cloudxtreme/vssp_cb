...
dojo.subscribe('addNodeSignal', function(e) {
	...
	try {
		if(e.status === 'SUCCESS') {
			...
			//loadContainerWithCameraDetails();
			//dataStore['refresh_home'] = 1;
			removeAllItemsInStack();
			loadContainerWithCameraDetails();
		} else {
...
clearAllFieldsInModifyNodeDialog = function() {
	...
	clearField('modifyVideoRecorderType');
	//clearField('addNodeVideoProfileID');
...
showModifyNodeDialog = function() {
	...
	var selectedNode = getCameraSelectedFromUserSettings();
...
fillDataInModifyDialog = function(node) {
	...
	setFieldValue('modifyVideoRecorderType', node.video_recorder_type);
	setFieldValue('modifyNodeVideoLocalStore', node.node_video_local_store);
	setFieldValue('modifyNodeDesc', node.node_desc);
...
modifyNode = function() {
	...
	nodeHandler.modifyNode(getValue('modifyNodeName'), 
							...
							getValue('modifyVideoRecorderType'),
...
showDeleteNodeDialog = function() {
	...
	*/
	if('admin_id' in dataStore) {
		setStatus('Admin has already logged in');
		userChoice = confirm('Do you want to delete the Camera:' + selectedNode.node_name + ' ?');
		if(userChoice) {
			deleteNode(selectedNode);
		} else {
			setStatus('Cancelled deletion of camera:' + selectedNode.node_name);
		}
	} else {
		setStatus('Admin authentication is not done');
		showAdminAuthenticateDlg();
